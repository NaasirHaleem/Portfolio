...
class BayesClassifier:
    ...
    def train(self) -> None:
        ...
        while o != 28000:
            m1 = str(m)
            try:
                d1 = self.load_file(self.training_data_directory+self.neg_file_prefix+"-"+m1+".txt")
                d2 = self.tokenize(d1)
                self.update_dict(d2,self.neg_freqs)
                o +=1
                m +=1
        ...
        while o != 28000:
            m1 = str(m)
            try:
                d1 = self.load_file(self.training_data_directory+self.pos_file_prefix+"-"+m1+".txt")
                d2 = self.tokenize(d1)
                self.update_dict(d2,self.pos_freqs)
                o +=1
                m +=1
            except FileNotFoundError:
                    print("No File Here")
    ...
    def classify(self, text: str) -> str:
        ...
        """
        a = self.load_dict(self.neg_filename)
        b = self.load_dict(self.pos_filename)
        ...
        aa = (sum(a.values())) #negative
        bb = (sum(b.values())) #postive
        ...
        ll = self.tokenize(text)

        for i in range(len(ll)):
           neg_prob = (a[ll[i]]+1)/aa
           log_neg_prob = math.log(neg_prob)
           sum_neg_prob += log_neg_prob
           pos_prob = (b[ll[i]]+1)/bb
           log_pos_prob = math.log(pos_prob)
           sum_pos_prob += log_pos_prob
        ...
        if sum_neg_prob < sum_pos_prob:
            sentiment = "positive"
            return sentiment
        elif sum_neg_prob == sum_pos_prob:
            sentiment = "neutral"
            return sentiment
        else:
            sentiment = "negative"
...
print(x.classify('This movie is good'))
